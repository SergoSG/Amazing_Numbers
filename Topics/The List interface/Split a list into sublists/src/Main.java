...
class ListUtils {
    ...
    public static <T> List<List<T>> splitListIntoSubLists(List<T> list, int subListSize) {
        ...
        for (int i = 0; list.size() > subListSize * i; i++) {
            int idx2 = i * subListSize + subListSize > list.size() - 1 ?
                       list.size() : i * subListSize + subListSize;
            sublists.add(list.subList(i * subListSize, idx2));
        }
